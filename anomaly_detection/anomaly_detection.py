"""This module is used to get gaussian anomaly detection"""
import numpy as np
...
class GaussianAnomalyDetection:
    ...
    def select_threshold(self, y_val: np.array, prob_val: np.array) -> float:
        ...
        epsilons = np.logspace(np.log10(min(prob_val)), np.log10(max(prob_val)), num=1000)
        ...
        print(f"Threshold was selected with the best epsilon: {self.epsilon} and the best f1: {best_f1}.")
